    ...
    try:
        ...
        for p_tag in p_tags:
            if 'salary:' in p_tag.text.lower():
                ...
                break
            if 'this position is no longer being advertised in people first.' in p_tag.text.lower():
...
def scrape_table_data(driver):
    KEYWORDS = ['analyst', 'programmer', 'specialist', 'data', 'computer',]
        ...
        for cell in td_cells:
            ...
            match cell.get_attribute('headers'):
                case 'hdrTitle':
                    ...
                    if not a or not any(keyword in a.text.lower() for keyword in KEYWORDS):
                        break
...
def main():
    ...
    try:
        ...
        result.to_csv(f'./PeopleFirstCareersOutput/results_{time.strftime("%Y_%m_%d_%H_%M_%S")}.csv')
    finally:
        driver.quit()
