...
def open_url_in_chrome(url, mode="headed"):
    ...
    d = webdriver.Firefox(
        service=Service(
            ...
            log_path=os.path.join(LOG_PATH, "yt_transcript.log"),
        ),
...
def get_transcript(driver: webdriver.Firefox, mode):
    ...
    try:
        # Click 'No thanks'
        show_transcript = driver.find_element(
            By.XPATH, "//button[@aria-label='No thanks']"
        )
    ...
    try:
        ...
        show_transcript = driver.find_element(
            By.XPATH, "//button[@aria-label='Show transcript']"
        )
        show_transcript.click()
    except Exception as e:
        pass
    ...
    transcript = ""
    try:
        transcript_element = driver.find_element(By.ID, "panels")
...
def transcript2df(transcript):
    ...
    df = pd.DataFrame(
        {
            "timestamp": transcript_timestamps[:min_len],
            "text": transcript_text[:min_len],
        }
    )
...
def main(url, mode="headless"):
    ...
    try:
        ...
        df.to_csv(
            os.path.join(
                PATH_TO_TRANSCRIPT,
                f"my_transcript_timestamped_{strftime('%Y-%m-%d-%H_%M_%S')}.csv",
            ),
            index=False,
        )
        with open(
            os.path.join(
                PATH_TO_TRANSCRIPT,
                f"my_transcript_text_only_{strftime('%Y-%m-%d-%H_%M_%S')}.txt",
            ),
            "w",
        ) as text_file:
...
if __name__ == "__main__":
    ...
    while (
        mode := input("select 1 for headed mode or 2 for headless mode: ")
    ) not in list("12"):
        print("please try again")
