...
// use each prime number p of the prime numbers found <= sqrt(N) to mark the
// multiples smallest multiple of p in the threads subrange
...
int main(int argc, char const *argv[]) {
  ...
  for (int i = 0; i < NUM_THREADS; i++) {
    ...
    // Create the thread
    if (pthread_create(&threads[i], NULL, markComposites,
                       (void *)&threadArgs[i]) != 0) {
